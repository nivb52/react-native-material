...
class ProductDetail extends React.PureComponent {
  ...
  render() {
    ...
    const renderHeader = () => (
      <View style={{ marginTop: HERO_HEIGHT }}>
        <View style={styles.titleContainer}>
          <Text id="pdcontent-title" style={styles.titleText}>
            {title}
          </Text>
        </View>
        <Text id="pdcontent-description" style={styles.descriptionText}>
          {description}
        </Text>
      </View>
    );
    const onPaletteReady = swatches => {
      if (swatches && swatches.length > 0) {
        ...
        const statusBarStyle =
          popularSwatch.theme === "light" ? "dark-content" : "light-content";
        this.setState({
          statusBarColor: popularSwatch.scrimColor,
          statusBarStyle,
          toolbarIconColor:
            popularSwatch.theme === "light"
              ? TOOLBAR_ICON_COLOR_DARK
              : TOOLBAR_ICON_COLOR_LIGHT
        });
      }
    };
    ...
    const fabStyle = [
      { top: initialFabTop },
      getFabStyleOnScroll(this._scrollY, titleHeight)
    ];
    return (
      <View style={styles.window}>
        ...
        <ListView
          dataSource={dsComments.cloneWithRows(comments)}
        ...
        <Toolbar
          icon="arrow-back"
          iconColor={this.state.toolbarIconColor}
          floating
        />
...
const Price = ({ price, style }) => (
  <LinearGradient
    colors={["#00000000", "#00000044", "#00000088"]}
    style={[style]}
  >
    <Transition.Text id="price" style={styles.priceText}>
      ${price}
    </Transition.Text>
  </LinearGradient>
);
  ...
  return new Promise((resolve, reject) => {
    ...
    UIManager.measureInWindow(tag, (x, y, width, height) => {
      if ([x, y, width, height].every(n => _.isNumber(n))) {
        ...
        reject(
          `Failed to measure ${tag}. x=${x}, y=${y}, width=${width}, height=${height}`
        );
      }
    });
  });
